...
def main():
    print("Welcome to the OCLC to Alma Workflow (Python Edition) - JOURNALS WORKFLOW")
    issn = input("Please enter the issn you want to search for: ").strip()
    
    if not issn:
        print("No issn entered. Exiting.")
        return
    ...
    if not alma_api_key:
        print("Error: Alma sandbox API key not found in alma_api_keys_github.txt")
        return
    ...
    print(f"\nChecking Alma via SRU to see if issn {issn} already exists...")
    ...
    try:
        ...
        if check_response.status_code == 200:
            ...
            if num_recs_elem is not None and num_recs_elem.text and num_recs_elem.text.isdigit():
                ...
                if total_records > 0:
                    print(f"Found {total_records} existing record(s) in Alma with issn {issn}.")
    ...
    try:
        ...
        # OCLC uses 'in' for issn queries
        journals = [{}]
        results, remaining = service.batch_search(journals, append_query=f"in:{issn}")
        ...
        if not eng_results:
            print("No English-cataloged records found in OCLC for that issn.")
            return
        ...
        print(f"Found {len(eng_results)} English-cataloged records. Validating 022 $a assignments...")
        ...
        for rec in eng_results:
            ...
            try:
                ...
                for f in list(temp_record):
                    if f.tag.endswith('datafield') and f.get('tag') == '022':
                        for s in list(f):
                ...
                if match_found:
                    ...
                    print(f"  Skipping {candidate_oclc} - Searched issn {issn} is not in 022 $a.")
            except Exception as e:
                print(f"  Error parsing XML for {candidate_oclc}: {e}")
                continue

        if not found_matches:
            print("No English-cataloged records had the requested issn explicitly assigned to 022 $a.")
            return
        ...
        print(f"\nSelected the highest scored record with a 022 $a match:")
...
# 
            ...
            try:
                with open('journal_record.xml', 'w', encoding='utf-8') as debug_f:
