...
def remove_elements(root, *tag_names):
    """Remove named child elements from XML root before creating in production."""
...
def get_authority(auth_id, sandbox_key):
    """Fetch full authority XML from Alma Sandbox."""
    url = f"{ALMA_API_BASE}/bibs/authorities/{auth_id}?view=full"
    response = requests.get(url, headers=sandbox_headers(sandbox_key), timeout=60)
...
def create_authority_prod(authority_xml, prod_key):
    """Create authority in Production and return newly assigned MMS ID."""
    root = ET.fromstring(authority_xml)
    ...
    response = requests.post(url, headers=prod_headers(prod_key), data=xml_body, timeout=60)
    response.raise_for_status()
...
def move_authority_sandbox_to_prod(auth_id, sandbox_key, prod_key):
    """Copy one authority record from Alma Sandbox to Alma Production."""
    LOG.info("Fetching authority %s from Sandbox...", auth_id)
    authority_xml = get_authority(auth_id, sandbox_key)

    heading = extract_heading(authority_xml)

    LOG.info("Creating authority in Production...")
    new_auth_id = create_authority_prod(authority_xml, prod_key)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Copy one authority record from Alma Sandbox to Alma Production.",
    )
...
def main():
    ...
    try:
        result = move_authority_sandbox_to_prod(auth_id, sandbox_key, prod_key)
    ...
    print(f"New production MMS ID: {result['new_auth_id']}")
    if result["heading"]:
